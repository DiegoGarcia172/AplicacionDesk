...
namespace AplicacionDesk
{
    public partial class FrmOrdenProduccion : Form
    {
        ...
        private async void btnPost_Click(object sender, EventArgs e)
        {
            ...
            {
                id = 0,
                fecha_Orden = dtpFechaOrden.Value.ToString("yyyy-MM-ddTHH:mm:ss.fffZ"),
                cantidad = int.TryParse(txtCantidad.Text, out var cantidadValue) ? cantidadValue : 0,
                fecha_Entrega = dtpFechaEntrega.Value.ToString("yyyy-MM-ddTHH:mm:ss.fffZ"),
                id_empleado = int.TryParse(txtIdEmpleado.Text, out var idEmpleadoValue) ? idEmpleadoValue : 0
            };
            bool success = await EjecutarPostPut(apiUrl, token, datos, HttpMethod.Post);
            MessageBox.Show(success ? "Orden de Producción creada exitosamente." : "No se pudo crear la Orden de Producción.");
        }
        private async void btnPut_Click(object sender, EventArgs e)
        {
            ...
            MessageBox.Show(success ? "Orden de Producción actualizada exitosamente." : "No se pudo actualizar la Orden de Producción.");
        }
