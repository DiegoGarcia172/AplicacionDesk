...
{
    public partial class FrmPostPutAlmacen : Form
    {
        public FrmPostPutAlmacen()
        {
            InitializeComponent();
        }
        ...
        private async void btnPost_Click(object sender, EventArgs e)
        {
            ...
            var datos = new
            {
                id = int.TryParse(txtid.Text, out var idValue) ? idValue : 0,
                nombre = txtName.Text,
                id_producto = int.TryParse(txtIdProducto.Text, out var idProductoValue) ? idProductoValue : 0,
                id_orden = int.TryParse(txtOrden.Text, out var idOrdenValue) ? idOrdenValue : 0
            };
            bool success = await EjecutarPostPut(apiUrl, token, datos, HttpMethod.Post);
            MessageBox.Show(success ? "Registro creado exitosamente." : "No se pudo crear el registro.");
        }
        private async void btnPut_Click(object sender, EventArgs e)
        {
            ...
            MessageBox.Show(success ? "Registro actualizado exitosamente." : "No se pudo actualizar el registro.");
        }
    }
}
