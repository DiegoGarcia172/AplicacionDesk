...
namespace AplicacionDesk
{
    public partial class FrmInventario : Form
    {
        ...
        private async void btnPost_Click(object sender, EventArgs e)
        {
            ...
            {
                id = 0,
                id_producto = int.TryParse(txtid.Text, out var idProductoValue) ? idProductoValue : 0,
                id_materiaprima = int.TryParse(txtIdMateriaPrima.Text, out var idMateriaPrimaValue) ? idMateriaPrimaValue : 0,
                id_almacen = int.TryParse(txtIdAlmacen.Text, out var idAlmacenValue) ? idAlmacenValue : 0
            };
            bool success = await EjecutarPostPut(apiUrl, token, datos, HttpMethod.Post);
            MessageBox.Show(success ? "Inventario creado exitosamente." : "No se pudo crear el inventario.");
        }
        private async void btnPut_Click(object sender, EventArgs e)
        {
            ...
            {
                id = idValue,
                id_producto = int.TryParse(txtIdProducto.Text, out var idProductoValue) ? idProductoValue : 0,
                id_materiaprima = int.TryParse(txtIdMateriaPrima.Text, out var idMateriaPrimaValue) ? idMateriaPrimaValue : 0,
                id_almacen = int.TryParse(txtIdAlmacen.Text, out var idAlmacenValue) ? idAlmacenValue : 0
            };
            string fullUrl = $"{apiUrl}/{idValue}";
            bool success = await EjecutarPostPut(fullUrl, token, datos, HttpMethod.Put);
            MessageBox.Show(success ? "Inventario actualizado exitosamente." : "No se pudo actualizar el inventario.");
        }
